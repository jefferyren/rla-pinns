"""Implements the Woodbury optimizer (CITATION TO COME) for PINNs."""
...
def parse_Woodbury_args(verbose: bool = False, prefix="Woodbury_") -> Namespace:
    """Parse command-line arguments for `Woodbury`.

    Args:
        verbose: Whether to print the parsed arguments. Default: `False`.
        prefix: The prefix for the arguments. Default: `'Woodbury_'`.

    Returns:
        A namespace with the parsed arguments.
    """
    parser = ArgumentParser(description="Parse arguments for setting up Woodbury.")

    parser.add_argument(
        f"--{prefix}lr", type=float, help="Learning rate for Woodbury.", required=True
    )
    parser.add_argument(
        f"--{prefix}damping",
        type=float,
    ...
    parser.add_argument(
        f"--{prefix}equation",
        type=str,
        choices=Woodbury.SUPPORTED_EQUATIONS,
    ...
    if verbose:
        print("Parsed arguments for Woodbury: ", args)

    return args


class Woodbury(Optimizer):
    """Woodbury optimizer for PINN problems.
    ...
    def __init__(
        ...
        if len(self.param_groups) != 1:
            raise ValueError("Woodbury does not support per-parameter options.")
    ...
    def step(
        ...
        # apply inverse of damped OOT to epsilon
        step = cholesky_solve(epsilon.unsqueeze(-1), cholesky(OOT)).squeeze(-1)
