"""Create a group plot of the high-dimensional Poisson results with Bayesian search."""
...
from kfac_pinns_exp.exp14_poisson_100d_weinan import plot as BIG
from kfac_pinns_exp.exp26_poisson5d_mlp_tanh_256_bayes import plot as SMALL
from kfac_pinns_exp.exp26_poisson5d_mlp_tanh_256_bayes.plot import colors, linestyles
from kfac_pinns_exp.exp32_poisson10d_mlp_tanh_256_bayes import plot as MEDIUM
    ...
    for (x, xlabel), (y, ylabel) in product(x_to_xlabel.items(), y_to_ylabel.items()):
        ...
        with plt.rc_context(
            ...
            for ax, exp in zip(axs, COLUMNS):
                D = exp.num_params
                title = f"{exp.dim_Omega}d {exp.equation.capitalize()}, "
                title += (
